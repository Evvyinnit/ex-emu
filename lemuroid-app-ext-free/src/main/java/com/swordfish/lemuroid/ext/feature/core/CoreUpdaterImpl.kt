...
class CoreUpdaterImpl(
    ...
    private suspend fun downloadCoreFromGithub(coreID: CoreID): File {
        ...
        val uri =
            if (coreID == CoreID.PLAY) {
                Uri.parse("https://buildbot.libretro.com/nightly/android/latest/")
                    .buildUpon()
                    .appendPath(Build.SUPPORTED_ABIS.first())
                    .appendPath("play_libretro_android.so.zip")
                    .build()
            } else {
                baseUri.buildUpon()
        ...
        try {
            if (coreID == CoreID.PLAY) {
                downloadAndExtractZip(uri, destFile)
            } else {
    ...
    private suspend fun downloadAndExtractZip(
        ...
        val tempZip = File(destFile.parentFile, "${destFile.name}.zip")
        try {
            downloadFile(uri, tempZip)
            java.util.zip.ZipInputStream(tempZip.inputStream()).use { zipStream ->
                val entry = zipStream.nextEntry
                if (entry == null) {
                    throw Exception("Empty core archive")
                }
